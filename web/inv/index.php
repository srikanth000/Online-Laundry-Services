...
{
mysql_query("insert into custumersmat(material,quan,total,inv_id,cus_id,ass_to,price)values('$itemName[$i]','$quantity[$i]','$total[$i]','$inv','".$cus_id."','$ass','$price[$i]')");
}
...
<div class="form-group">
<select name="ass" id="ass"><option>Select User</option><?php $sql=mysql_query("select * from users where status=2"); while($row=mysql_fetch_array($sql)){?><option value="<?php echo $row['user_id'];?>"><?php echo $row['first_name'];?></option><?php } ?></select>
      		</div>
      	</div>
      	<h2>&nbsp;</h2>
      	<div class='row'>
